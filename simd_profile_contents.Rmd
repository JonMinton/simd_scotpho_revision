...
pacman::p_load(tidyverse, plotly, crosstalk)
```
...
As described in [the SIMD 2020 technical notes](https://www.gov.scot/publications/simd-2020-technical-notes/), the weighting of indicators into items is determined through factor analysis, whereas the weighting of domains to the overall score is based on expert judgement. 
...
```
**Chart 1**: Spearman correlation of SIMD 2020 domain ranks and overall rank.
...
Chart 2 shows the how the 15% most deprived datazones in Scotland are distributed across NHS Health boards. The slider at the bottom can be moved to show how the proportions vary by level of deprivation (e.g. 0.25 shows the share of datazones in the most deprived 35% of Health Boards). 

```{r, echo = FALSE, include = FALSE}

dta %>% 
  mutate(pct_rank = SIMD2020v2_Rank / max(SIMD2020v2_Rank)) %>% 
  mutate(is_top_15pc = pct_rank < 0.15) %>% 
  group_by(HBname) %>% 
  summarise(pct_top_15pc = mean(is_top_15pc)) %>% 
  ungroup() %>% 
  ggplot(aes(x = pct_top_15pc, y = reorder(HBname, pct_top_15pc))) + 
  geom_bar(stat = "identity") +
  labs(
    x = "Percent in top 15%", 
    y = "Health Board",
    title = "Percent of datazones in health boards in most deprived 15% of population"
  )

# Now to attempt the same in plotly 
...
# Code chunk from website https://plotly.com/r/sliders/#basic-slider-control
...
dep_steps <- list()
fig <- plot_ly() 
for (i in c(3, 1, 2, 4:n_steps)) { # Start with 3 as this is 15% and this should determine the default LA order 
 fig <- add_bars(fig,x=dep_vals[i][[1]]$x,  y=dep_vals[i][[1]]$y, visible = dep_vals[i][[1]]$visible, 
                 name = dep_vals[i][[1]]$name, orientation = 'h', hoverinfo = 'x+y', color = I("gray"),
                 showlegend = FALSE) %>% 
   layout(
      title = list(
        text = glue::glue("Proportion of datazones in LAs at least this deprived")
      ),
      xaxis = list(
        title = "Proportion this deprived in Local Authority",
        range = list(0, 1)
      ),
      yaxis = list(
...
**Chart 3**: The proportion of datazones in each local authority which are in the 15% most deprived small areas in Scotland. (The slider can be used to select other deprivation thresholds. Source: SIMD 2020)
...
Data are also available for the SIMD 2004, 2006, 2009, 2012 and 2016.  [PHS Data and Intelligence](http://www.isdscotland.org/Products-and-Services/GPD-Support/Deprivation/SIMD) (formerly ISD Scotland) have also published detailed look-up files for SIMD 2004, 2006, 2009, 2012 and 2016.
...
The website above, formerly from ISD Scotland, has a section on deprivation with useful pages with recommendations for analysing health data, ways to categorise deprivation variables and choices of geographical unit for small area analysis.

[Scottish Indices of Deprivation 2003](http://www.isdscotland.org/Products-and-Services/GPD-Support/Deprivation/)

The precursor to the SIMD were the [Scottish Indices of Deprivation 2003](https://www.researchgate.net/publication/265271331_Scottish_Indices_of_Deprivation_2003), produced by Oxford University on behalf of the Scottish Executive. Similar to SIMD 2004, but using slightly different domains and indicators, this deprivation measure was calculated at electoral ward level. An overall index, domain-specific indices, report and maps are all available from the relevant page of the Scottish Executive web site (see link above).
